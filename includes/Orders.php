...
class Orders {
	...
	public function maybe_create_invoice( $order_id, $order_old_status, $order_new_status ) {
		...
		if ( $order_new_status === $this->integration->get_option( 'invoice_sync_status', 'processing' ) && 'yes' !== $this->integration->get_option( 'invoice_sync_onhold', 'no' ) ) {
			$create_invoice = true;
		}

		if ( 'on-hold' !== $order_old_status && $order_new_status === $this->integration->get_option( 'invoice_sync_status', 'processing' ) && 'yes' === $this->integration->get_option( 'invoice_sync_onhold', 'no' ) ) {
			$create_invoice = true;
		}

		if ( 'on-hold' === $order_new_status && 'yes' === $this->integration->get_option( 'invoice_sync_onhold', 'no' ) ) {
			$create_invoice = true;
		}

		if ( 'processing' !== $order_old_status && 'completed' === $order_new_status && ! $invoice_id ) {
			$create_invoice = true;
		}

		$create_invoice = apply_filters( 'wc_' . $this->get_plugin()->get_id() . '_create_invoice', $create_invoice, $order, $invoice_id );

		if ( true === $create_invoice ) {
	...
	public function validate_cart_products_quantity( &$data, &$errors ) {
		if ( 'yes' === $this->integration->get_option( 'stock_sync_allowed', 'no' ) && 'yes' === $this->integration->get_option( 'validate_checkout_stock', 'no' ) ) {
			foreach ( WC()->cart->get_cart() as $cart_item ) {
				...
				if ( $product ) {
					...
					if ( $update_stock ) {
						$product_quantities = $this->integration->get_product_from_lookup_table( $product->get_sku() );
						$total_quantity     = ! empty( $product_quantities ) ? array_sum( wp_list_pluck( $product_quantities, 'stock_quantity' ) ) : 0;

						$product->set_stock_quantity( $total_quantity );
						$product->save();

						if ( $total_quantity <= 0 && ! $product->is_on_backorder() ) {
